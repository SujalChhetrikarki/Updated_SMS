...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) $subjectsAll[] = $row;
}
?>
<!DOCTYPE html>
<html lang="en">
...
.sidebar h2 { text-align:center; margin-bottom:30px; font-size:22px; color:#3b82f6; }
.sidebar a {
    display:flex; align-items:center; padding:12px 20px; margin:4px 12px;
    background:#374151; color:#fff; text-decoration:none; border-radius:8px;
...
form button { width:100%; padding:12px; margin-top:20px; background:#3b82f6; color:#fff; font-weight:500; border:none; border-radius:8px; cursor:pointer; transition:0.3s; }
form button:hover { background:#2563eb; }

/* ===== Class & Subject boxes ===== */
.class-box { border:1px solid #e5e7eb; padding:10px; border-radius:8px; margin-bottom:12px; background:#fafafa; }
.class-box .class-title { font-weight:500; margin-bottom:6px; }
.class-box .subjects-list label { display:block; font-weight:400; margin-bottom:3px; }
...
/* ===== Notes ===== */
.note { font-size:13px; color:#6b7280; margin-top:5px; }
...
<div class="card">
    ...
    <form action="add_teacher_process.php" method="POST" id="addTeacherForm">
        ...
        <label><input type="checkbox" id="is_class_teacher" name="is_class_teacher" value="1"> Make this teacher a Class Teacher (only one class)</label>

        <div id="class_teacher_select" style="display:none; margin-top:10px;">
            <label for="class_teacher_class">Select Class (Class Teacher):</label>
        ...
        <?php foreach($classes as $class): ?>
            <div class="class-box">
                <div class="class-title">
                    <label>
                        <input type="checkbox" class="teaching_class_checkbox" name="teaching_classes[]" value="<?= $class['class_id']; ?>">
                        <?= htmlspecialchars($class['class_name']); ?>
                    </label>
                </div>
                <div class="subjects-list" style="margin-left:18px; margin-top:6px;">
                    <?php foreach($subjectsAll as $sub): ?>
                        <label><input type="checkbox" name="subjects_for_class[<?= $class['class_id']; ?>][]" value="<?= $sub['subject_id']; ?>"> <?= htmlspecialchars($sub['subject_name']); ?></label>
                    <?php endforeach; ?>
                </div>
            </div>
        <?php endforeach; ?>
    ...
    <p class="note">Class Teacher role is limited to one class. Teacher can still teach subjects in multiple classes.</p>
</div>
</div>

<script>
// Show/hide class teacher select
...
// Ensure subjects require class checkbox
document.getElementById('addTeacherForm').addEventListener('submit', function(e){
    ...
    boxes.forEach(box => {
        ...
        let anySub = false;
        subjects.forEach(s => { if(s.checked) anySub = true; });
        if(anySub && !classChk.checked){ valid = false; classChk.focus(); }
    });
    if(!valid){ e.preventDefault(); alert("If you select subjects for a class, make sure the corresponding class checkbox is checked."); }
});
</script>
